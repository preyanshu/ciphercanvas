...
export default function EligibilityCheck({ artwork }: EligibilityCheckProps) {
  ...
  const checkEligibility = async () => {
    ...
    try {
      ...
      // If submitted winner, compute 50% of escrow as SOL amount
      if (hasSubmitted) {
        try {
  ...
  const claimReward = async () => {
    ...
    try {
      ...
      if (error.message?.includes("plugin closed") || error.message?.includes("user rejected")) {
        ...
        errorMessage = "Vote verification failed - you did not vote for the winning proposal"
    ...
    <div>
        {!hasChecked ? (
          <div className="text-center">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full border-2 border-accent flex items-center justify-center">
              <Gift className="w-8 h-8 text-accent" />
            </div>
            <h4 className="font-bold text-lg text-foreground font-mono mb-2">CHECK YOUR ELIGIBILITY</h4>
            <p className="text-sm text-muted-foreground font-mono mb-6">See if you qualify for this artwork's rewards</p>
            
            <Button
              onClick={checkEligibility}
              disabled={isChecking}
              className="w-full bg-accent text-black font-mono font-bold text-sm px-6 py-3 hover:bg-accent/90"
            >
              {isChecking ? (
                <>
          ...
          <div className="space-y-4">
            {eligibilityData.isEligible ? (
              <div className="text-left">
                ...
                {/* Rewards Section */}
                <div className="mb-4">
                  <p className="text-xs font-bold text-foreground font-mono mb-3">REWARDS</p>
                  <div className="bg-muted/20 rounded-sm border border-border/30 p-4 space-y-3">
                    {/* NFT Artwork Reward */}
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-sm overflow-hidden border border-border/30">
                        <img
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <h4 className="text-sm font-bold text-foreground font-mono">{eligibilityData.rewardAmount > 0 ? `${eligibilityData.rewardAmount.toFixed(4)} SOL` : "0 SOL"}</h4>
                        <p className="text-xs text-muted-foreground font-mono">{eligibilityData.submittedWinner ? "50% of total donations for this round" : "Share of 50% voter pool (split among all voters)"}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <Button
                  onClick={claimReward}
                  className="w-full bg-accent hover:bg-accent/90 text-black font-mono text-sm font-bold px-6 py-3"
                >
                  CLAIM REWARDS
                </Button>
              </div>
            ) : (
              <div className="border border-red-500/50 rounded-sm p-4 bg-red-500/5 text-left">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-8 h-8 bg-red-500/20 rounded-full flex items-center justify-center flex-shrink-0">
                    <XCircle className="w-4 h-4 text-red-500" />
                  </div>
                  <h3 className="text-sm font-bold text-foreground font-mono">NOT ELIGIBLE</h3>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-xs text-muted-foreground font-mono">Voted for this artwork</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <div className="w-1.5 h-1.5 bg-muted-foreground rounded-full mt-1.5 flex-shrink-0"></div>
                    <span className="text-xs text-muted-foreground font-mono">Submitted this artwork</span>
                  </div>
                </div>
              </div>
            )}
