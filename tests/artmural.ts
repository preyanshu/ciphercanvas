...
const USE_DEVNET = process.env.USE_DEVNET === "true" || process.argv.includes("--devnet");

// Helper function to retry RPC calls with fresh blockhash (like devnet-anchor.js)
async function retryRpcCall<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // If account already exists, that's okay - just return success (like devnet-anchor.js)
      if (error.message?.includes("already in use") || 
          error.message?.includes("0x0") || // Anchor "AccountDiscriminatorAlreadySet" error
...
describe("artmural", () => {
  ...
  let program: Program<ProposalSystem>;
  let provider: anchor.AnchorProvider;
  let connection: Connection;
  let clusterAccount: PublicKey;

  // Set up provider and program based on environment
  before(async () => {
    if (USE_DEVNET) {
      ...
      connection = new Connection(DEVNET_RPC_URL, DEVNET_COMMITMENT);
      
      // Load owner keypair
      const ownerKeypairPath = "/home/preyanshu/test-wallet.json";
      const ownerKeypair = Keypair.fromSecretKey(
        new Uint8Array(JSON.parse(fs.readFileSync(ownerKeypairPath, "utf8")))
      );
      
      // Create wallet and provider
      const wallet = new anchor.Wallet(ownerKeypair);
      provider = new anchor.AnchorProvider(connection, wallet, {
        commitment: DEVNET_COMMITMENT,
      });
      ...
      // Create program instance exactly like in devnet-anchor.js
      // Use the hardcoded program ID (the IDL metadata should have the program ID, but we verify it)
      const programId = new PublicKey(PROGRAM_ID);
      program = new Program(idl, provider) as Program<ProposalSystem>;
      
      // Verify program ID matches
      if (program.programId.toBase58() !== PROGRAM_ID) {
        console.warn(`⚠️  Warning: IDL program ID (${program.programId.toBase58()}) doesn't match expected (${PROGRAM_ID})`);
        console.warn("   This might cause issues. Consider rebuilding with: anchor build");
      }
      
      // Devnet configuration - use cluster account
      ...
      // Test basic program access first (like devnet-anchor.js)
      try {
        ...
        if (programInfo) {
          ...
          console.warn("⚠️  Program not found on devnet - make sure it's deployed");
        }
      } catch (error: any) {
        console.warn("⚠️  Program access test failed:", error.message);
      }
    } else {
      console.log("🏠 Running tests on LOCALNET");
      console.log("=".repeat(60));
      
      // Configure for local cluster (default Anchor behavior)
      const envProvider = anchor.AnchorProvider.env();
      anchor.setProvider(envProvider);
      program = anchor.workspace.ProposalSystem as Program<ProposalSystem>;
      provider = envProvider;
      connection = provider.connection;
      
      // For localnet, cluster account will be set up during initialization
      clusterAccount = PublicKey.default; // Will be set later
    }
  });
  ...
  // Helper function to get the cluster account based on environment
  const getClusterAccount = (): PublicKey => {
    if (USE_DEVNET) {
  ...
  before(async () => {
    const owner = readKpJson("/home/preyanshu/test-wallet.json");
  ...
  it.only("can initialize the proposal system", async () => {
    ...
    const round1OpenThemeProposals = [
      ...
      },
      {
        ...
        url: "https://res.cloudinary.com/dbo7hzofg/image/upload/v1761850778/ChatGPT_Image_Oct_31_2025_12_29_09_AM_hhhqkx.png",
      },
    ];
    ...
    const round0HistorySig = await retryRpcCall(async () => {
      return await program.methods
        .createRoundHistory("Neon Phoenix")
        .accounts({
          payer: owner.publicKey,
     ...
     const round2NeonPhoenixProposals = [
      {
        ...
        url: "https://res.cloudinary.com/dbo7hzofg/image/upload/v1761850056/Screenshot_from_2025-10-31_00-15-36_wg8od1.png",
      },
      {
    ...
    for (let i = 3; i < 6; i++) {
       const { title: proposalTitle, description: proposalDescription, url: proposalUrl } = round2NeonPhoenixProposals[i-3];
      console.log(`Submitting: ${proposalTitle} - ${proposalDescription}`);
    ...
    const round1HistorySig = await retryRpcCall(async () => {
      return await program.methods
        .createRoundHistory("Quantum Beast")
        .accounts({
          payer: owner.publicKey,
          roundHistory: PublicKey.findProgramAddressSync(
            [
    ...
    const round2ProposalSig = await program.methods
      .submitProposal("Entangled Howl", "QubitFang", "https://res.cloudinary.com/dbo7hzofg/image/upload/v1761850256/Screenshot_from_2025-10-31_00-20-12_ibpsvt.png")
      .accountsPartial({
        payer: owner.publicKey,
    ...
    console.log(`✅ Round 2 proposal submitted: ${round2ProposalSig}`);
      ...
      const round2ProposalSig1 = await program.methods
      .submitProposal("Superposition Serpent", "DecoWave", "https://res.cloudinary.com/dbo7hzofg/image/upload/v1761851516/Screenshot_from_2025-10-31_00-41-18_jmarr4.png")
      .accountsPartial({
        payer: owner.publicKey,
        roundEscrow: round2EscrowPDA,
      })
    ...
    console.log(`✅ Round 2 proposal submitted: ${round2ProposalSig}`);
      ...
      const round2ProposalSig2 = await program.methods
      .submitProposal(" Schrödinger’s Titan", "CollapseTheory", "https://res.cloudinary.com/dbo7hzofg/image/upload/v1761851678/Screenshot_from_2025-10-31_00-44-20_crpr3i.png")
      .accountsPartial({
        payer: owner.publicKey,
        roundEscrow: round2EscrowPDA,
      })
    ...
    // Assertions
    expect(finalSystemAcc.nextProposalId).to.equal(9) // 0,1,2 + 3,4,5 + 6 (test proposal)
...
console.log("This will never run");
  ...
  async function initProposalVotesCompDef(
    ...
    // Check if computation definition already exists (like devnet-anchor.js pattern)
    try {
      const compDefAccount = await program.provider.connection.getAccountInfo(compDefPDA);
      if (compDefAccount) {
